...
def publish_data(sender, instance, created, **kwargs):
    ...
    try:
        ...
        # gdf.drop('geometry', 1, inplace=True) # Drop the geometry column (since we already backup this colum with geom)
    ...
    '''
    geo.create_featurestore(store_name= 'geo', workspace= 'geoapp', db= 'geoapp', host= 'localhost', pg_user= 'postgres', pg_password='postgres', schema='data')

    geo.publish_featurestore(workspace='geoapp', store_name='geo', pg_table=name)
...
def delete_data(sender, instance, **kwargs):
    ...
    # geo.delete_layer('test10', 'geoapp')
